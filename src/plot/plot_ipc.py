...
def get_umlc_miss_ratio(descriptor_data, sim_path, output_dir):
    benchmarks_org = descriptor_data["workloads_list"].copy()
    benchmarks = []
    umlc_miss_ratio = {}
            ...
            for benchmark in benchmarks_org:
                ...
                with open(exp_path + config_key + '/memory.stat.0.csv') as f:
                    ...
                    for line in lines:
                        ...
                        elif 'MLC_HIT_count' in line and 'DC_PREF_REQ_DCACHE_HIT_count' not in line \
                ...
                if umlc_hits + umlc_misses > 0:
                    umlc_miss_ratio_val = umlc_misses / (umlc_hits + umlc_misses)
                else:
                    umlc_miss_ratio_val = 0.0  # Handle edge case where both are 0

                avg_umlc_miss_ratio += umlc_miss_ratio_val
                cnt_benchmarks += 1

                if len(benchmarks_org) > len(benchmarks):
                    benchmarks.append(benchmark_name)

                umlc_miss_config.append(umlc_miss_ratio_val)
                print(f"Benchmark: {benchmark_name}, Misses: {umlc_misses}, Hits: {umlc_hits}, Miss Ratio: {umlc_miss_ratio_val}")

            # Calculate average after all benchmarks
            if cnt_benchmarks > 0:
                avg_umlc_miss_ratio /= cnt_benchmarks
                umlc_miss_config.append(avg_umlc_miss_ratio)
                umlc_miss_ratio[config_key] = umlc_miss_config

        benchmarks.append('Avg')
        print("UMLC Miss Ratio Data:", umlc_miss_ratio)
        plot_data(benchmarks, umlc_miss_ratio, 'UMLC Miss Ratio', output_dir + '/FigureC.png')
...
'''
def plot_stacked_bar(benchmarks, data, ylabel_name, fig_name):
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c']  # Colors for the miss types
    miss_types = ['DCACHE_COMPULSORY', 'DCACHE_CAPACITY', 'DCACHE_CONFLICT']

    # Preparing the data for stacking
    totals = np.array([np.sum(data[key], axis=0) for key in data.keys()])
    ...
    for i, miss_type in enumerate(miss_types):
        ax.bar(ind, totals[:, i], width, label=miss_type, bottom=bottom, color=colors[i])
        bottom += totals[:, i]  # Update the bottom for stacking
...
if __name__ == "__main__":
    ...
    get_umlc_miss_ratio(descriptor_data, args.simulation_path, args.output_dir)
